...
看完了？那么你可以开始了。如果你啥都没有，可以到群里下载更好用的 QQ 魔改版或使用 QQLite。
...
* 实在不行刷个 GSI 吧。刷入GSI以后也需要按照第三条修改。
...
可以，请参考 [这个教程](https://bbs.midrai.cn/d/140) 进行扩容。 parted 工具若无法下载请见群文件或见回帖中的补档。
...
5. 确保设备管理器可以看到端口，如果bypass以后断开了链接需要重新bypass
6. 点击 `Download`，等待刷写完成。此时 Bootloader 被回锁；
7. 在 MTK Client 中执行命令 `python mtk da seccfg unlock`，将设备连接电脑以进入 Download Mode，等待解锁完成
8. 断开数据线，使用 电源+复读+红键+音量上 按键组合进入 TWRP Recovery；
9. 转到 `Wipe`，滑动滑块；
10. 点击 `Format Data`，输入 `Yes` 并点击确定，以防止 Recovery 被覆盖；
11. 转到 `Advanced` -> `Reboot`，点击 `Power Off`；
12. 长按电源键约 10s 以开机
...
7. 进入 mtkclient 目录，替换 `Recovery` 为 `TWRP Recovery`；，在 MTK Client 中执行命令 `python mtk da seccfg unlock`，将设备连接电脑以进入 Download Mode，等待解锁完成
8. 在当前目录打开终端，执行:
   ```shell
...
使用twrp来刷入 `system` 镜像，这本身是没有大问题的，**但是！twrp不能识别 `Sparse Image` !** twrp能做到的只是把镜像里的0101二进制数据一个字不漏的抄进 `system` 分区。

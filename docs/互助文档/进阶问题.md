...
另请注意，部分账号由于被腾讯风控，无法在旧版本上登陆，对于此情况无解。（目前qq在官网上放了8.8.90-32位的版本，猜测很快就会解除风控）
...
小爱本身没有重力传感器和旋转传感器，没法自动旋转。但可以在酷安下载强制旋转屏幕，roation（好用，推荐）进行手动旋转，或者下载 MyGesture 或 button mapper 进行设置快捷转屏键。
...
这个办法仅仅适用于已经root的用户，但是很多人不大喜欢root或者经常被救砖模块把谷歌模块冻住，那该怎么办呢？
BitGapps，完美解决你的痛苦，目前已经在官方系统和lineageOS17上体验过了，没有任何问题
lineageOS17.1：https://bbs.midrai.cn/d/149
官方：https://bbs.midrai.cn/d/73
...
！！！！首先，给出警告，没有熟练掌握进阶Q21中的救砖方法的清远离本帖子，由于扩容之中的变数套多，暂时不建议小白进行扩容
~~可以。对于 Wifi 版，你可以下载 [这个工具（github.com/CrackMiDrAi/SystemPartitionExtendingToolForArchytas）](https://github.com/CrackMiDrAi/SystemPartitionExtendingToolForArchytas) 来一键扩容。~~ 该工具目前被删除，请参考 [这个教程](https://bbs.midrai.cn/d/140) 进行扩容。 parted 工具若无法下载请见群文件或见回帖中的补档。（这个帖子里面的东西基本都没了，在这里补一下命令生成的网址https://caner.hk/ExtendingSystem/）（parted：https://bbs.midrai.cn/d/167）
...
12. 长按电源键约 10s 以开机
### SP Flash Tool 刷入法（汉化版）

1. 在关机状态按住音量下并将设备连接电脑以进入Download Mode，使用 mtk-bypass 工具进行 bypass；
2. 打开 SP Flash Tool，选择 `格式化`，在 `自动格式化标记` 区域中选中 `格式化整个闪存（不包含Bootloader）`，并点击 `开始`；
3. 完成后，断开数据线，长按电源键约 10s（无画面显示），按住音量下连接电脑以进入 Download Mode，使用 mtk-bypass 工具进行 bypass。若未识别到设备，请重复操作；
4. 选择 `下载`，在 `配置文件` 处选择救砖包的 `MT6761_Android_scatter.txt`，并替换 Recovery 镜像为 TWRP Recovery（个人认为可替换可不替换，后续再刷就是。也可以前往官网下载编译完成的橙狐，PBRP等其他rec）；
    * **【重要】确认 `验证文件` 下的选框中选中的为 `下载`，而非 `全部格式化和下载`，否则将造成 Red State；**
6. 点击 `下载`，等待刷写完成。此时 Bootloader 被回锁；
7. 断开数据线，使用 电源+复读+红键+音量上 按键组合进入 TWRP Recovery；
8. 转到 `清除`，滑动滑块；
9. 点击 `格式化Data分区`，输入 `Yes` 并点击确定，以防止 Recovery 被覆盖；
10. 转到 `重启`，点击 `关机`；
11. 找到你下载的破解包，打开解锁bl文件夹，点击!!启动mtkclient命令行.bat，输入 `python mtk da seccfg unlock`，等待出现`Port-Hint`之后，将设备在关机状态下，按住音量下连接电脑以进入 Download Mode，等待解锁完成
12. 长按电源键约 10s 以开机
### 手动刷入法
...
* 本教程是我在扩容失败之后嫌弃vmware开的client速度太慢，0.3mb/s你敢信，而sp在刷入是会有一部分分区刷不进去而且很小，加起来也不到50mb，我就想着把这部分分区用client刷入，其他的分区用sp进行刷入
1. 在关机状态按住音量下并将设备连接电脑以进入Download Mode，使用 mtk-bypass 工具进行 bypass；
2. 打开 SP Flash Tool，选择 `格式化`，在 `自动格式化标记` 区域中选中 `格式化整个闪存（不包含Bootloader）`，并点击 `开始`；
3. 完成后，断开数据线，长按电源键约 10s（无画面显示），按住音量下连接电脑以进入 Download Mode，使用 mtk-bypass 工具进行 bypass。若未识别到设备，请重复操作，出现报错，请多尝试几次，不行重启再试，最后去检查驱动；
4. 选择 `下载`，在 `配置文件` 处选择救砖包的 `MT6761_Android_scatter.txt`；
    * **【重要】确认 `验证文件` 下的选框中选中的为 `下载`，而非 `全部格式化和下载`，否则将造成 Red State；**
5. 从这里开始就是本教程与其他的不同的开始，在上一步的bypass完了之后 ，点击下载开始刷入，刷到md1img这个分区一定会有报错，不要管他报错什么，拿出你的小本本，把他记下来，然后再做一遍第三步，取消勾选md1img分区，再点下载，没有报错就直接开机，如果有报错，把报错的分区记录下来，反复执行这个过程，直到最后一次性过了；
6. 还记得你在第五步记录下来的那些刷不过去的分区不，打开mtkclient（直接打开解锁bl文件夹就好），把那些分区对应的文件，不管是.img,还是.bin,都拖到解锁bl文件夹内，然后一个个进行刷入就比如要刷入md1img这个分区，命令就是这样 python mtk w md1img 对应的文件拖过来。把这些分区都刷进来。没有意外就可以开机了。是在不行可以找我QQ:1375091474；
根据我个人经验，应该是md1img，spmfw，scp1，scp2，sspm_1,sspm_2，lk1，lk2，logo，tee1，tee2这些分区需要重新刷
...
常规方法（刷入原厂系统后再使用 TWRP 刷入 LOS）刷入 LOS GSI 时会卡 TWRP，现给出特殊方法。（请先尝试常规方法，一般是没什么事的，特殊方法不建议不熟悉mtkclient的人食用）

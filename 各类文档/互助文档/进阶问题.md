...
~~可以。对于 Wifi 版，你可以下载 [这个工具（github.com/CrackMiDrAi/SystemPartitionExtendingToolForArchytas）](https://github.com/CrackMiDrAi/SystemPartitionExtendingToolForArchytas) 来一键扩容。~~
该工具目前被删除，请参考 [这个教程](https://bbs.midrai.cn/d/140) 进行扩容。 parted 工具若无法下载请见群文件。**注意**：建立分区完毕后在 parted 工具中执行 `p` 命令确认分区是否成功被命名，**若没有请使用 `name` 命令手动命名**。另外，若 fastboot 无法使用，请手动在设备管理器中安装驱动或换用 mtkclient 刷入镜像。4G版机型建议在扩容前先备份一下 `nvram` `nvdata` `nvcfg`分区，以防丢串。
...
11. 在 MTK Client 中执行命令 `python mtk xflash seccfg unlock`，将设备连接电脑以进入 Download Mode，等待解锁完成
12. 长按电源键约 10s 以开机
...
8. 在当前目录打开终端，执行:
   ```shell
   python mtk w recovery,md1img,spmfw,scp1,scp2,sspm_1,sspm_2,cache,user
   ta,vbmeta,lk,lk2,boot,logo,dtbo,tee1,tee2,vendor,system .\recovery.img,.\md1rom.img,.\spmfw.img,.\tinysys-scp1.bin,.\tinysys-scp2.bin,.\sspm.img,.\sspm.img,.\userdata.img,.\userdata.img,.\vbmeta.img,.\lk.bin,.\lk2.bin,.\boot.img,.\logo.bin,.\dtbo.img,.\trustzone1.bin,.\trustzone2.bin,.\vendor.img,.\system.img
   ```
...
8. 在当前目录打开终端，执行:
      ...
      ```
9. 刷写完成后，拔掉数据线，在终端输入 `python mtk xflash seccfg unlock`，等待出现 `Port Hint...` 后再插入数据线以解锁BL

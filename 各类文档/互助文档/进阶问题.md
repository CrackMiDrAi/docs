...
13. * **热泉的特殊救砖方法**
   ...
   10. 每一行刷写完成，拔掉数据线，输入下一行命令，等待出现`Port Hint`再插入数据线
   11. 断开数据线，使用 电源+复读+红键+音量上 按键组合进入 TWRP Recovery；
   12. 转到 `Wipe`，滑动滑块；
   13. 点击 `Format Data`，输入 `Yes` 并点击确定，以防止 Recovery 被覆盖；
   14. 转到 `Advanced` -> `Reboot`，点击 `Power Off`；
   15. 在 MTK Client 中执行命令 `python mtk da seccfg unlock`，将设备连接电脑以进入 Download Mode，等待解锁完成
   16. 长按电源键约 10s 以开机

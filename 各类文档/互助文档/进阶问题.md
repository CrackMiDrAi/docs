...
1. 冻结推荐黑洞，删除推荐 ADB 或者 `Archytas-Tools`。
...
3. 方法3：
    * 用 `Archytas-Tools` 工具打开壁纸服务，安全性最高。
...
该工具目前被删除，请参考 [这个教程](https://bbs.midrai.cn/d/140) 进行扩容。 parted 工具若无法下载请见群文件。**注意**：建立分区完毕后在 parted 工具中执行 `p` 命令确认分区是否成功被命名，**若没有请使用 `name` 命令手动命名**。另外，若 fastboot 无法使用，请手动在设备管理器中安装驱动或换用 mtkclient 刷入镜像。此外，无论4G版还是Wifi版，建议都在扩容前先使用sp回读一下nvram nvdata nvcfg分区，以防丢失串号/SN/Wifi mac/蓝牙 mac。可回读版sp见群文件，为防止意外，我同样在[Onedrive](https://tzmsky-my.sharepoint.com/:u:/g/personal/tzmsky0302_tzmsky_top/EXf-blkLKxJNnEHghIzm-5ABFu_ocfppUxj8svqUhHIK2Q?e=FWzFvy)扔了一份，需要自取。

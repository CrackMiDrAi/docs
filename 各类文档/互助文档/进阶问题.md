...
8. 在当前目录打开终端，依次执行:
      ...
      python mtk w lk,lk2,boot,logo,dtbo,tee1,tee2,vendor,system .\lk.bin,.\lk2.bin,.\boot.img,.\logo.bin,.\dtbo.img,.\trustzone1.bin,.\trustzone2.bin,.\vendor.img,.\system.img
      ```
    * **每一行回车后请按下小爱音量下并插入数据线**
9. 每一行命令刷写完成后，拔掉数据线，输入下一行命令，等待出现 `Port Hint...` 再插入数据线
10. 全分区刷写完毕后，拔掉数据线，在终端输入 `python mtk da seccfg unlock` 解锁BL
11. 之后请进入TWRP进行root操作，TWRP以及Los可正常使用
